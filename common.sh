...
func_schema_setup() {
  if [ "${schema_type}" == "mongodb" ]; then
    ...
    mongo --host mongodb.sowmyakotnidevops.online </app/schema/${component}.js &>>${log}
    func_exit_status
  fi
  ...
  if [ "${schema_type}" == "mysql" ]; then
    ...
    mysql -h mysql.sowmyakotnidevops.online -uroot -pRoboShop@1 < /app/schema/${component}.sql &>>${log}
    func_exit_status
  fi
